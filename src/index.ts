...
/**
 * Azure Voice Live Proxy Server
 *
 * Transparent WebSocket proxy for Azure Voice Live API with two main responsibilities:
...
/**
 * WebSocket proxy endpoint for Azure Voice Live
 *
 * Mode is automatically inferred from URL parameters:
 *
 * Standard Mode (Voice/Avatar):
...
app.get("/", (_req, res) => {
  res.json({
    name: "@iloveagents/azure-voice-live-proxy",
    version: "1.1.0",
    description: "Secure WebSocket proxy for Azure Voice Live API",
    endpoints: {
      health: "GET /health",
      websocket: "WS /ws",
    },
    documentation: "https://github.com/iLoveAgents/azure-voice-live-proxy",
  });
});
